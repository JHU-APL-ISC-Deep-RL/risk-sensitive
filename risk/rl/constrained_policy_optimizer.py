...
from risk.common.utils import get_network_object
from risk.common.samplers import compute_entropy
from risk.common.mpi_data_utils import average_grads, sync_weights, mpi_statistics_scalar, mpi_avg,\
...
class ConstrainedPolicyOptimizer(PolicyOptimizer):
        ...
        if self.config['use_prior_nets']:
            if self.mode == 'test' and self.config['test_iteration'] is not None:
                if self.config['test_iteration'] < 0:
                    ...
                    torch.load(self.penalty_param, os.path.join(self.config['model_folder'], 'penalty-' +
    ...
    def update_cost_values(self, observations):
        """  Estimate cost values with updated cost value network, store in buffer  """
        self.buffer.cost_values = self.c_v_network(observations).view(-1).detach().numpy()
